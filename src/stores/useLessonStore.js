...
const useLessonStore = create((set, get) => ({
  ...
  fetchLessons: async (filters = {}) => {
    ...
    try {
      ...
      const response = await axiosInstance.get(`/lessons?${queryParams}`);
      ...
      const errorMessage = error.response?.data?.message || "فشل في تحميل الدروس";
  ...
  fetchLessonStats: async () => {
    ...
    try {
      ...
      set({ lessonStats: response.data.data, isLoading: false });
